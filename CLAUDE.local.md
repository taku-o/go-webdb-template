...
* 【絶対厳守】 認証エラーは確認したとは言わない。確認できかった、というステータスであり、確認が必要なタスクなら、そのタスクは未完了です。
...
### データ分散環境での常識
* テーブル分割する環境では外部キー制約を使わない。
    * 同じデータベース内に参照先のデータがなかったり、あるいはその制約のせいでデータを別のデータベースに移動できないことがあるため。
* テーブルのIDをデータを一意として扱うためのキーとして使わない。
    * そのような場合は、テーブル内に、例えばuser_idのような別のidentifierを用意する。
