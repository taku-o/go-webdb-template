...
describe('apiClient', () => {
  ...
  describe('CSV Download API', () => {
    ...
    beforeEach(() => {
      ...
      mockAppendChild = jest.spyOn(document.body, 'appendChild').mockImplementation(() => mockLink) as jest.Mock
      mockRemoveChild = jest.spyOn(document.body, 'removeChild').mockImplementation(() => mockLink) as jest.Mock
    })
  ...
  describe('Today API', () => {
    it('gets today date (uses getAuthToken internally)', async () => {
      ...
      expect(result).toEqual(mockResponse)
      expect(authModule.getAuthToken).toHaveBeenCalled()
      expect(fetch).toHaveBeenCalledWith(
