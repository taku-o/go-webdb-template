...
**重要**: 複数のAPIサーバープロセスが同時に動作している状態は問題を引き起こす可能性があります。

各APIサーバーはAsynqサーバーを内蔵しており、Redisに自身を登録します（`asynq:servers:{hostname:pid:uuid}`）。複数のサーバーが動いていると：

- ジョブが複数のサーバーに分散処理される
- どのサーバーの標準出力にジョブ結果が出るか予測できない
- 古いプロセスが残っていると、ジョブが「消えた」ように見える
