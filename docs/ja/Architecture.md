...
Adminアプリのカスタムページ（dm_user_register.go、api_key.go）は、APIサーバーと同じレイヤー構造（pages → usecase → service → repository → db）を採用しています。
...
│  │  • DmUserRegisterUsecase: ユーザー登録ビジネスロジック   │
│  │  • APIKeyUsecase: APIキー発行ビジネスロジック           │
│  └──────────────────────┬─────────────────────────────────┘ │
│                         │                                     │
│  ┌──────────────────────▼─────────────────────────────────┐ │
│  │              Service Layer (internal/service)           │ │
│  │  • DmUserService: ユーザードメインロジック              │
│  │  • APIKeyService: APIキードメインロジック               │
│  └──────────────────────┬─────────────────────────────────┘ │
│                         │                                     │
│  ┌──────────────────────▼─────────────────────────────────┐ │
│  │           Repository Layer (internal/repository)        │ │
│  │  • DmUserRepository: ユーザーデータアクセス            │
│  └──────────────────────┬─────────────────────────────────┘ │
│                         │                                     │
│  ┌──────────────────────▼─────────────────────────────────┐ │
│  │                 DB Layer (internal/db)                  │ │
│  │  • GroupManager: シャーディング接続管理                 │
│  └──────────────────────┬─────────────────────────────────┘ │
└─────────────────────────┼───────────────────────────────────┘
                          │
         ┌────────────────┴────────────────┐
...
- `auth/`: 認証・セッション管理
- `pages/`: カスタムページ（ダッシュボード、ユーザー登録、APIキー発行）

**Location**: `internal/usecase/admin/`

- `dm_user_register_usecase.go`: ユーザー登録ビジネスロジック
- `api_key_usecase.go`: APIキー発行ビジネスロジック

**Location**: `internal/service/`

- `api_key_service.go`: APIキー発行ドメインロジック
...
4. **カスタムページ**: ダッシュボード、ユーザー登録フォーム、APIキー発行
