**[日本語]** | [English](../en/Command-Line-Tool.md)
...
開発環境で起動しているサーバーの状態を確認するためのツールです。13個のサーバー（API、Client、Admin、JobQueue、PostgreSQL、MySQL、Redis、Redis Cluster、Mailpit、CloudBeaver、Superset、Metabase、Redis Insight）のポート番号にTCP接続を試行し、起動状態を判定して表形式で表示します。
...
|---------|-------|------|
| API | 8080 | APIサーバー |
| Client | 3000 | Next.js開発サーバー |
| Admin | 8081 | 管理サーバー |
| JobQueue | 8082 | ジョブキューサーバー |
| PostgreSQL | 5432 | PostgreSQLデータベース |
| MySQL | 3306 | MySQLデータベース |
| Redis | 6379 | Redisキャッシュ・ジョブキュー |
| Redis Cluster | 7100 | Redis Cluster（最初のポート） |
| Mailpit | 8025 | メールテストツール |
| CloudBeaver | 8978 | データベース管理ツール |
| Superset | 8088 | データ可視化ツール |
| Metabase | 8970 | BIツール |
| Redis Insight | 8001 | Redis管理ツール |

### 技術仕様

- **確認方法**: TCP接続（`net.DialTimeout`）
- **タイムアウト**: 1秒
- **並列実行**: goroutineを使用して全サーバーを並列に確認
- **外部依存**: なし（標準ライブラリのみ使用）
