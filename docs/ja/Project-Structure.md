**[日本語]** | [English](../en/Project-Structure.md)
...
- **App Router**: ページルーティング（`app/`ディレクトリ）
- **Components**: 再利用可能なUIコンポーネント（shadcn/uiを含む）
- **Lib**: API呼び出しやユーティリティ関数
- **Types**: TypeScript型定義
- **認証**: NextAuth (Auth.js) v5による認証
...
- `/` - トップページ（機能一覧、認証状態表示）
- `/dm-users` - ユーザー一覧・作成・編集・削除、CSVダウンロード
- `/dm-posts` - 投稿一覧・作成・編集・削除
- `/dm-user-posts` - ユーザーと投稿のJOIN結果表示（クロスシャードクエリ）
- `/dm_email/send` - メール送信ページ
- `/dm_movie/upload` - 動画アップロードページ（TUSプロトコル）
- `/dm-jobqueue` - ジョブキューページ
